...
void message_handler(const char* cmd, const char* param) {
    printf("[DEBUG] message_handler() : cmd='%s', param='%s'\n", cmd, param);
    fflush(stdout);
...
void* client_handler(void* arg) {
    ...
    printf("[DEBUG] Client %d en attente de messages...\n", client_id);
    fflush(stdout);

    // Send welcome message
    printf("Sending welcome message to client %d\n", client_id);
    if (communication && communication->comX) {
        communication->comX(communication, "Welcome to the server!");
    } else {
        printf("[ERROR] Failed to send welcome message to client %d\n", client_id);
    }
...
// Function to send a custom message to a client
void send_custom_message(const char* message) {
    pthread_mutex_lock(&clients_mutex);
    
    // Send the message to all connected clients
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (server_communications[i]) {
            printf("[DEBUG] Sending custom message to client %d: %s\n", i, message);
            server_communications[i]->comY(server_communications[i], message);
        }
    }
...
// Function to handle user input for custom messages
void* custom_message_input(void* arg) {
    char message[256];
...
int main() {
    ...
    // Create a thread for handling custom messages input
    pthread_t input_thread;
    if (pthread_create(&input_thread, NULL, custom_message_input, NULL) != 0) {
