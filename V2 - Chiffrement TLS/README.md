...
> **Version :** 0.1  |  **Date :** 1 juin 2025  
> **Auteurs :** *[À compléter : prénom nom – affiliation]*
...
| **Modèle multi-thread** | `pthread` + un thread par client | Simplicité ; performances suffisantes (< 20 conn. simult.) ; OpenSSL thread-safe après appel `OPENSSL_init_ssl` |
...
* **Classes clés** :
  * `SSLUtil` : chargement `server.crt` (assets) → `KeyStore` → `TrustManager`.
  * `Connection` : lecture/écriture I/O avec timeout.
  * `Communication` : thread de réception + callback UI.
  * `Protocol` : encodage/décodage `CMD|PARAM`.
  * `MainActivity` : UI (IP, logs, boutons *Connect/Send/Disconnect*).
...
* **CI/CD** : pipeline GitHub Actions → tests unitaires + analyse Sonar.
* **Hardening** : activation HSTS côté serveur, ré-génération clé / cert. via Ansible.
